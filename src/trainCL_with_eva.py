...
class MultieupDataset(Dataset):
    ...
    def validate_alignment_queries(self, alignment_queries):
        invalid_query_ids = []
        for query_id, content in alignment_queries.items():
            if not isinstance(content, dict) or 'parallel_queries' not in content or not content['parallel_queries']:
                invalid_query_ids.append(query_id)
                continue
            for parallel_query in content['parallel_queries']:
        ...
        for query_id in invalid_query_ids:
            del alignment_queries[query_id]
        ...
        if query_id in self.alignment_queries and 'query' not in self.alignment_queries[query_id]:
            ...
            self.alignment_queries[query_id] = {
                'query': self.tokenizer(query, padding='max_length', truncation=True, max_length=self.max_query_length,
                                        return_tensors='pt'),
                'parallel_queries': {
                    k: self.tokenizer(v, padding='max_length', truncation=True, max_length=self.max_query_length,
                                      return_tensors='pt')
...
def compute_parallel_loss(dataset, query_ids, query_embeddings, model, device, passage_embeddings):
    ...
    for query_id in query_ids:
        ...
        if 'parallel_queries' not in alignment_query:
            # print(f"Skipping query_id {query_id} due to missing 'parallel_queries'")
            continue

        # Sample a parallel query (second element in each parallel_queries list is the query text)
        parallel_query_text = random.choice(alignment_query['parallel_queries'])[1]
        ...
        # Normalize parallel query embedding
        parallel_query_embedding = normalize_embeddings(parallel_query_embedding)
        ...
        kl_loss = F.kl_div((similarity_distribution_parallel + epsilon).log(),
                           similarity_distribution_original + epsilon, reduction='batchmean')

        total_parallel_loss += kl_loss

    # Return the average parallel loss over all queries
    return total_parallel_loss / len(query_ids)
...
def compute_metrics(output_path, languages, epoch):
    ...
    results_df.to_csv(f'{output_path}/epoch_{epoch}_results/ir_metrics_mdpr_ep{epoch}.csv', float_format='%.3f')


def compute_correlation(output_path, languages, epoch):
    cutoff_k = 5
    rank_lists_by_lang = {lang: [] for lang in languages}
    ...
    analysis_path = os.path.join(output_path, f'epoch_{epoch}_results/Analysis')
...
def evaluate_model(args, model, epoch, device, logger, languages, tokenizer, sorted_qid_df, alignment_file,
    ...
    results_dir = os.path.join(args.dev_input_dir, f'epoch_{epoch}_results')
    ...
    compute_metrics(args.dev_input_dir, languages, epoch)
    compute_correlation(args.dev_input_dir, languages, epoch)
...
def main(args):
    ...
    if args.mode in ["train", "train+evaluate"]:
        logger.info("Loading and preprocessing data for training...")
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=total_training_steps * 0.1,
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for step, (queries, passages, query_ids) in enumerate(
                ...
                if not stop_alignment_loss:
                    parallel_loss = compute_parallel_loss(dataset, query_ids, query_embeddings, model, device,
                                                          passage_embeddings)
                ...
                if step % args.log_steps == 0:
                    logger.info(
                        f"Epoch [{epoch + 1}/{args.epochs}], Step [{step}/{len(dataloader)}], DPR Loss: {dpr_loss.item():.4f}, Alignment Loss: {parallel_loss.item():.8f}, Combined Loss: {loss.item():.4f}")
